...
Generated pinning helpers such as `usePinThingMutation.fetcher` route to this endpoint automatically and require an `apikey` header. Keep this key on a server. Browser apps should call their own server route instead of embedding the key in client-side code.
...
Pin a "Thing" object (general entity) to IPFS. The raw GraphQL mutation returns an IPFS URI:
...
The generated package helper uses the same endpoint routing:
...
configureClient({ pinApiKey: process.env.INTUITION_PIN_API_KEY })

const uri = await requestPinThing({
  name: 'TypeScript Programming Language',
  description:
...
8. **Cache metadata** - Store IPFS URI for future reference

**Error Handling Considerations**:

- Mutation may fail if image classification fails (inappropriate content)
- IPFS pinning may fail without a valid pinning API key or may timeout - implement retry logic
