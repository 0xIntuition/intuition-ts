**@0xintuition/sdk**

---

# @0xintuition/sdk

## Interfaces

- [CostEstimation](interfaces/CostEstimation.md)
- [GlobalSearchOptions](interfaces/GlobalSearchOptions.md)
- [SemanticSearchOptions](interfaces/SemanticSearchOptions.md)
- [SyncConfig](interfaces/SyncConfig.md)
- [WaitOptions](interfaces/WaitOptions.md)

## Type Aliases

- [AtomWithId](type-aliases/AtomWithId.md)
- [BatchCreateAtomsFromThingsOptions](type-aliases/BatchCreateAtomsFromThingsOptions.md)
- [BatchCreateAtomsFromThingsResult](type-aliases/BatchCreateAtomsFromThingsResult.md)
- [CreateAtomConfigWithIpfs](type-aliases/CreateAtomConfigWithIpfs.md)
- [CreateAtomFromThingOptions](type-aliases/CreateAtomFromThingOptions.md)
- [CreateAtomFromThingResult](type-aliases/CreateAtomFromThingResult.md)
- [PinataPinResponse](type-aliases/PinataPinResponse.md)
- [PinThingOptions](type-aliases/PinThingOptions.md)
- [SdkConfig](type-aliases/SdkConfig.md)
- [SearchResult](type-aliases/SearchResult.md)
- [TripleWithIds](type-aliases/TripleWithIds.md)

## Functions

- [batchCreateAtomsFromEthereumAccounts](functions/batchCreateAtomsFromEthereumAccounts.md)
- [batchCreateAtomsFromIpfsUris](functions/batchCreateAtomsFromIpfsUris.md)
- [batchCreateAtomsFromSmartContracts](functions/batchCreateAtomsFromSmartContracts.md)
- [batchCreateAtomsFromThings](functions/batchCreateAtomsFromThings.md)
- [batchCreateTripleStatements](functions/batchCreateTripleStatements.md)
- [batchDeposit](functions/batchDeposit.md)
- [batchRedeem](functions/batchRedeem.md)
- [calculateAtomId](functions/calculateAtomId.md)
- [calculateCounterTripleId](functions/calculateCounterTripleId.md)
- [calculateTripleId](functions/calculateTripleId.md)
- [configureSdk](functions/configureSdk.md)
- [createAtomFromEthereumAccount](functions/createAtomFromEthereumAccount.md)
- [createAtomFromIpfsUpload](functions/createAtomFromIpfsUpload.md)
- [createAtomFromIpfsUri](functions/createAtomFromIpfsUri.md)
- [createAtomFromSmartContract](functions/createAtomFromSmartContract.md)
- [createAtomFromString](functions/createAtomFromString.md)
- [createAtomFromThing](functions/createAtomFromThing.md)
- [createTripleStatement](functions/createTripleStatement.md)
- [deposit](functions/deposit.md)
- [findAtomIds](functions/findAtomIds.md)
- [findTripleIds](functions/findTripleIds.md)
- [getAtomDetails](functions/getAtomDetails.md)
- [getTripleDetails](functions/getTripleDetails.md)
- [globalSearch](functions/globalSearch.md)
- [pinThing](functions/pinThing.md)
- [redeem](functions/redeem.md)
- [search](functions/search.md)
- [semanticSearch](functions/semanticSearch.md)
- [sync](functions/sync.md)
- [uploadJsonToPinata](functions/uploadJsonToPinata.md)
- [wait](functions/wait.md)
