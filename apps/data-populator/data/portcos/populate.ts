...
const superscrypt = await loadCsvToSchemaType<Thing>(
  'src/portcos/data/superscrypt.csv',
)
const polymorphic = await loadCsvToSchemaType<Thing>(
  'src/portcos/data/polymorphic.csv',
)
const spaceship = await loadCsvToSchemaType<Thing>(
  'src/portcos/data/spaceship.csv',
)
...
const superscryptTag: WithContext<Thing> = {
  ...
  description:
    'Superscrypt is an early stage crypto-native VC whose mission is to onboard the next wave of builders and users into web3. We invest in category-defining web3 teams, and partner with them to scale their business. Our focus is on infrastructure and emerging use cases; including identity & credentials, developer tools, data indexing & search, scaling and privacy. Superscrypt is made up of founders who have decades of experience building and scaling technology businesses, and we leverage this experience to help our teams with GTM, product, technology, tokenomics, strategy & community building.',
  image:
    'https://www.superscrypt.xyz/wp-content/uploads/2022/05/Superscrypt-Logo-normal.png',
...
const polymorphicTag: WithContext<Thing> = {
  ...
  image:
    'https://crypto-fundraising.info/wp-content/uploads/funds/2022/04/Polymorphic-Capital.png',
