...
export async function precomputeCID<T extends Thing>(obj: WithContext<T>): Promise<string> {
    try {
...
export async function checkCIDPinned(cid: string): Promise<boolean> {
    try {
        const response = await fetch(`${ipfs}/ipfs/${cid}?pinataGatewayToken=${gatewayToken}`);
        return response.ok;
    } catch (error) {
        console.log(error);
    }
...
export async function checkObjectPinned(obj: any, requestHash?: string): Promise<[boolean, string]> {
    try {
        const cid = await precomputeCID(obj);
        requestHash ? pushUpdate(requestHash, `Precomputed CID for atom data - checking if ${cid} is already pinned...`) : null;
