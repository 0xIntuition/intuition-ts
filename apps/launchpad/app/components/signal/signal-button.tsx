...
const SignalButton: React.FC<SignalButtonProps> = ({
  ...
  const formattedCurrentSharePrice = formatUnits(
    BigInt(current_share_price),
    18,
  )
  const formattedShares = formatUnits(BigInt(shares), 18)
      ...
      onClick={(e) => {
        ...
        if (
          ...
          if (
            mode === 'deposit'
              ? +val > +walletBalance
              : +val > +formattedShares * +formattedCurrentSharePrice
          ) {
